...
public class ActionableToastBar extends LinearLayout {
    ...
    public ActionableToastBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        mBottomMarginSizeInConversation = context.getResources().getDimensionPixelSize(R.dimen.toast_bar_bottom_margin_in_conversation);
     ...
     */
    public void show(final ActionClickedListener clickListener, final IOnHideToast hideListener, int descriptionIconResourceId,
     ...
     */
    public void hide(IOnHideToast listener, boolean animate) {
        ...
        if (!mHidden && !getShowAnimation().isRunning()) {
            ...
            if (getVisibility() == View.VISIBLE) {
                ...
                if (animate) {
                    getHideAnimation(listener).start();
                } else {
                    setAlpha(0);
    ...
    private Animator getHideAnimation(final IOnHideToast listener) {
        if (mHideAnimation == null) {
            ...
            mHideAnimation.addListener(new Animator.AnimatorListener() {
                ...
                @Implement public void onAnimationStart(Animator animation) {}

                @Implement public void onAnimationRepeat(Animator animation) {}

                @Implement
                public void onAnimationEnd(Animator animation) {
                    if (listener != null) listener.onViewDisappear();
                    setVisibility(View.GONE);

                }

                @Implement public void onAnimationCancel(Animator animation) {}
            });
