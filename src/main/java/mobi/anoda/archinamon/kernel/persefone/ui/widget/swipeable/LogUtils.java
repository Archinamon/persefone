...
public class LogUtils {
    ...
    public static final  String  TAG                                 = "Persefone";
    // "GMT" + "+" or "-" + 4 digits
    ...
    /**
     * Used to enable/disable logging that we don't want included in production releases.
     */
     ...
     * @param args   the list of arguments passed to the formatter. If there are more arguments than required by {@code format}, additional arguments are ignored.
     */
    ...
    /**
     * What a Terrible Failure: Report a condition that should never happen. The error will always be logged at level ASSERT with the call stack. Depending on system configuration, a report may be added to the {@link android.os.DropBoxManager} and/or the process may be terminated immediately with an
     * error dialog.
     *
     * @param tag    Used to identify the source of a log message. It usually identifies the class or activity where the log call occurs.
     * @param format the format string (see {@link java.util.Formatter#format})
     ...
     * <p/>
     * It fixes: - "Thu, 10 Dec 09 15:08:08 GMT-0700" to "Thu, 10 Dec 09 15:08:08 -0700" (4 digit zone value can't be preceded by "GMT") We got a report saying eBay sends a date in this format
     */
    public static String cleanUpMimeDate(String date) {
        if (TextUtils.isEmpty(date)) {
            return date;
        }
        date = DATE_CLEANUP_PATTERN_WRONG_TIMEZONE.matcher(date)
                                                  .replaceFirst("$1");
        return date;
    }
