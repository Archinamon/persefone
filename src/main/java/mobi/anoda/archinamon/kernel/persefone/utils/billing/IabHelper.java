...
package mobi.anoda.archinamon.kernel.persefone.utils.billing;
...
/**
 * Provides convenience methods for in-app billing. You can create one instance of this class for your application and use it to process in-app billing operations. It provides synchronous (blocking) and asynchronous (non-blocking) methods for many common in-app billing operations, as well as
 ...
 * <p/>
 * When you are done with this object, don't forget to call {@link #dispose} to ensure proper cleanup. This object holds a binding to the in-app billing service, which will leak unless you dispose of it correctly. If you created the object on an Activity's onCreate method, then the recommended place
 * to dispose of it is the Activity's onDestroy method.
 * <p/>
 * A note about threading: When using this object from a background thread, you may call the blocking versions of methods; when using from a UI thread, call only the asynchronous versions and handle the results via callbacks. Also, notice that you can only call one asynchronous operation at a time;
 * attempting to start a second asynchronous operation while the first one has not yet completed will result in an exception being thrown.
 *
...
public class IabHelper {
     ...
     * @param base64PublicKey Your application's public key, encoded in base64. This is used for verification of purchase signatures. You can find your app's base64-encoded public key in your application's page on Google Play Developer Console. Note that this is NOT your "developer public key".
     */
    ...
    public interface OnIabPurchaseFinishedListener {
        ...
        /**
         * Called to notify that an in-app purchase finished. If the purchase was successful, then the sku parameter specifies which item was purchased. If the purchase failed, the sku and extraData parameters may or may not be null, depending on how far the purchase process went.
         *
    ...
    /**
     * Initiate the UI flow for an in-app purchase. Call this method to initiate an in-app purchase, which will involve bringing up the Google Play screen. The calling activity will be paused while the user interacts with Google Play, and the result will be delivered via the activity's {@link
     * Activity#onActivityResult} method, at which point you must call this object's {@link #handleActivityResult} method to continue the purchase flow. This method MUST be called from the UI thread of the Activity.
     *
     * @param act         The calling activity.
     * @param sku         The sku of the item to purchase.
     * @param itemType    indicates if it's a product or a subscription (ITEM_TYPE_INAPP or ITEM_TYPE_SUBS)
     * @param requestCode A request code (to differentiate from other responses -- as in {@link Activity#startActivityForResult}).
    ...
    /**
     * Handles an activity result that's part of the purchase flow in in-app billing. If you are calling {@link #launchPurchaseFlow}, then you must call this method from your Activity's {@link Activity@onActivityResult} method. This method MUST be called from the UI thread of the Activity.
     *
    ...
    /**
     * Queries the inventory. This will query all owned items from the server, as well as information on additional skus, if specified. This method may block or take long to execute. Do not call from a UI thread. For that, use the non-blocking version {@link #queryInventoryAsync}.
     *
     * @param querySkuDetails if true, SKU details (price, description, etc) will be queried as well as purchase information.
     * @param moreItemSkus    additional PRODUCT skus to query information on, regardless of ownership. Ignored if null or if querySkuDetails is false.
     * @param moreSubsSkus    additional SUBSCRIPTIONS skus to query information on, regardless of ownership. Ignored if null or if querySkuDetails is false.
     *
    ...
    /**
     * Asynchronous wrapper for inventory query. This will perform an inventory query as described in {@link #queryInventory}, but will do so asynchronously and call back the specified listener upon completion. This method is safe to call from a UI thread.
     *
    ...
    /**
     * Consumes a given in-app product. Consuming can only be done on an item that's owned, and as a result of consumption, the user will no longer own it. This method may block or take long to return. Do not call from the UI thread. For that, see {@link #consumeAsync}.
     *
